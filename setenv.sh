...
## ka09 OS. Source this into your current environment by running:
## . ./setenv.sh
##
...
case "$ARCH$VARIANT" in
    x86q)     target=i586-pc-ka09 ;;
    x86)      target=i686-pc-ka09 ;;
    armv[67]) target=arm-none-ka09 ;;
    x64)      target=x86_64-none-ka09 ;;
...
run 'make tools' in $SRCROOT/third-party."
        ...
        echo "Warning: Failed to find tools$failed_tools. You may need to \
download the latest ka09 toolchain for your build OS, or rebuild the tools
from source by grabbing the third-party repository and running 'make tools'."
    fi
else
    echo "ka09 build environment for $ARCH$VARIANT."
fi
